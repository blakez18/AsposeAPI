...
namespace EPPService.Service
{
    public class EPlusPlus
    {
        ...
        public static ExcelPackage CreatingChart(ExcelPackage ex) // Create Chart code
        {
            ...
            int lastRowPos = ex.Workbook.Worksheets[1].Cells.Where(cell => !cell.Value.ToString().Equals("")).Last().End.Row-1;
            int lastColumnPos = ex.Workbook.Worksheets[1].Cells.Where(cell => !cell.Value.ToString().Equals("")).Last().End.Column-1;
